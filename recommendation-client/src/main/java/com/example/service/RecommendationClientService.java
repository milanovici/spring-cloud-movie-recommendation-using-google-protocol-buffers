...
import com.example.proto.ReccomendationDataProtos;
...
//    }
    ...
    public CompletableFuture<UserProtos.User> getProtoUserDetails(Long userId) {
        LOGGER.info("Looking for user with id: " + userId.toString());
        CompletableFuture<UserProtos.User> future = CompletableFuture.supplyAsync(() -> {
            ParameterizedTypeReference<UserProtos.User> ptr
                    = new ParameterizedTypeReference<UserProtos.User>() {
            };
            ResponseEntity<UserProtos.User> user = restTemplate.exchange("http://user-service/user/proto/" + userId.toString(), HttpMethod.GET, null, ptr);
            LOGGER.info("I've found a your guy!");
            return user.getBody();
        });
        return future;
    ...
    public CompletableFuture<RecommendationDTO> getProtoRecommendationData(Long userId) {
        LOGGER.info("Requesting recommendation for user with id: " + userId.toString());
        ParameterizedTypeReference<ReccomendationDataProtos.ReccomendationDataList> ptrRecommendedMovie
                = new ParameterizedTypeReference<ReccomendationDataProtos.ReccomendationDataList>() {
        };
        ResponseEntity<ReccomendationDataProtos.ReccomendationDataList> recommendedMovies
                = restTemplate.exchange("http://recommendation-service/recommendation/proto/recommend/user/" + userId.toString(), HttpMethod.GET, null, ptrRecommendedMovie);

        LOGGER.info("Ok, lets recommend you something!");
        RecommendationDTO data = new RecommendationDTO();

        List<Integer> ids = recommendedMovies.getBody().getDataList().stream().map(ReccomendationDataProtos.ReccomendationData::getId).collect(Collectors.toList());
        CompletableFuture<RecommendationDTO> test = getProtoUserDetails(userId).thenCombine(getProtoMovies(ids), (user, movies) -> {
            UserDTO udto = new UserDTO();
            udto.setId(user.getId());
            udto.setName(user.getName());
            data.setUser(udto);
            movies.getMovieList().stream().forEach((recommendation) -> {
                String name = recommendation.getName();
                Long movieId = recommendation.getMovieId();
                Integer likes = recommendedMovies.getBody()
                        .getDataList()
                        .stream()
                        .filter(movie -> movie.getId() == movieId.intValue())
                        .findFirst()
                        .get()
                        .getLikes();
                data.getMovies().add(new MovieDTO(movieId.toString(), name, likes));
    ...
    public CompletableFuture<MovieProtos.MovieList> getProtoMovies(Collection<Integer> ids) {
        LOGGER.info("Looking for movies with ids: " + ids.toString());
        Gson gson = new Gson();
        String idsString = gson.toJson(ids);
        CompletableFuture<MovieProtos.MovieList> future = CompletableFuture.supplyAsync(() -> {
            ParameterizedTypeReference<MovieProtos.MovieList> ptrMovie
                    = new ParameterizedTypeReference<MovieProtos.MovieList>() {
            };
            ResponseEntity<MovieProtos.MovieList> movies
                    = restTemplate.exchange("http://movie-service/movie/proto/list?ids={ids}", HttpMethod.GET, null, ptrMovie, idsString);
            LOGGER.info("I've found some movies!");
            return movies.getBody();
        });
        return future;
